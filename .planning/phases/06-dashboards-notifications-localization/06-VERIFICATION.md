...
status: human_needed
score: 12/12 must-have truths verified (roadmap success criteria); 1 of 2 non-blocking code-review warnings resolved with automated proof, 1 remains open pending an infrastructure-level finding
...
human_verification:
  - test: "Inspect the WebSocket frames (or React DevTools state) delivered to the browser for a `postgres_changes` INSERT event on `public.notifications`."
    ...
    why_human: "Quick task 260827-isc applied `supabase/migrations/20260827120000_scope_notifications_realtime_columns.sql` (drop + re-add `public.notifications` on `supabase_realtime` with an explicit 6-column list), live via `db push --linked`, and independently confirmed via `select attnames from pg_publication_tables where tablename = 'notifications'` that the Postgres catalog is now correctly scoped to exactly `{id,user_id,type,related_appointment_id,read_at,created_at}` — `message` is genuinely absent from the publication definition. However, a new wire-level Playwright test (`tests/e2e/notifications-realtime.spec.ts`, \"notifications publication withholds the message column from the wire\", currently `test.fixme`) failed on 5 consecutive live runs spanning several minutes: the decoded `postgres_changes` payload still carried `message` every time, despite each run using a brand-new channel and a brand-new sentinel value (ruling out a client-side or test-level cache). This points at the managed Realtime service's own CDC decoder (a `wal2json`-based logical replication slot, confirmed via `pg_replication_slots`) either caching the pre-ALTER column list independently of Postgres's catalog, or not honoring publication column-list restrictions for this decoding path at all — neither is something the CLI/application layer available to this executor can fix or force-refresh (no CLI subcommand exists to restart/inspect the Realtime tenant's internal state). A human decision is needed: accept the Postgres-catalog-level fix as the delivered mitigation with wire-level exposure remaining an open, documented residual risk pending Supabase-side investigation (support ticket or dashboard project restart), or treat this as blocking further phase sign-off until wire-level delivery is independently reconfirmed."
---
...
**Verified:** 2026-08-13T19:30:00Z
**Status:** human_needed
**Re-verification:** Yes — 2026-08-27T00:00:00Z (quick task 260827-isc): WR-01 confirmed resolved (already fixed, no new edit); WR-02 partially addressed (Postgres catalog-level fix applied and confirmed live) but remains open — a new wire-level test found the Realtime service still delivers the withheld column, an infrastructure-level finding outside this session's ability to resolve. Status stays `human_needed` per this report's own precondition (line ~150 of the original report): both warnings must resolve before the status can flip to `passed`, and only one has.
...
**Score:** 12/12 roadmap-level truths verified. 0 behavior-unverified. Of the 2 non-blocking code-review findings routed to human verification: WR-01 is now resolved with commit + automated-test proof (see `resolved_human_verification` in frontmatter); WR-02 remains open below — the Postgres catalog is now correctly scoped, but a new wire-level test (quick task 260827-isc) found the managed Realtime service still delivers the withheld column, an infrastructure-level finding this session could not resolve from the CLI/application layer.
...
None blocking. Two non-blocking findings carried from the code review (`06-REVIEW.md`, status `issues_found`, 0 critical / 3 warnings / 2 info — advisory-only per the review gate), one of which is now resolved:
...
| `supabase/migrations/20260812090000_enable_notifications_realtime.sql` | 14 | No column-list on `alter publication ... add table public.notifications` | ⚠️ Warning (WR-02) — **PARTIALLY RESOLVED, still open** | `supabase/migrations/20260827120000_scope_notifications_realtime_columns.sql` (quick 260827-isc) re-scopes the publication to 6 explicit columns, confirmed correct at the Postgres catalog level (`pg_publication_tables`). A new wire-level test still observes `message` in the live decoded payload after 5 consecutive runs — an apparent Realtime-service-side (not Postgres-side) propagation gap, outside this session's ability to fix from the CLI |
...
1. ~~**Notification mark-as-read local state sync (WR-01)**~~ — **RESOLVED, no longer requires human verification.**
...
2. **Realtime publication column scope (WR-02) — still open, partially addressed**
   ...
   **What changed (quick task 260827-isc, 2026-08-27):** `supabase/migrations/20260827120000_scope_notifications_realtime_columns.sql` re-scopes the `supabase_realtime` publication to exactly 6 columns (drop + re-add with an explicit list), applied live via `db push --linked` and independently confirmed correct via a direct `pg_publication_tables` query on the linked project — `message` is genuinely absent from the Postgres publication catalog now.
   **Why still human:** A new wire-level test (`tests/e2e/notifications-realtime.spec.ts`, "notifications publication withholds the message column from the wire", currently `test.fixme`) failed 5 consecutive live runs spanning several minutes — the decoded `postgres_changes` payload still carried `message` every time, with a fresh channel and fresh sentinel per run (ruling out client/test-level caching). This points at the managed Realtime service's `wal2json`-based CDC decoder either caching the pre-ALTER schema independently of Postgres's catalog, or not honoring publication column-list restrictions for this decoding path at all. Neither is fixable from the CLI/application layer available to this executor — there is no CLI subcommand to restart or inspect the Realtime tenant's internal state. Needs a decision: accept the Postgres-catalog-level fix as the delivered mitigation with wire-level exposure as a documented residual risk pending Supabase-side investigation, or continue blocking phase sign-off on this item until wire-level delivery is independently reconfirmed (e.g., after a project restart via the Supabase dashboard, or a longer-elapsed re-test in a future session).
...
Of the two code-review warnings that gated `human_needed` status: **WR-01 is now resolved** with a commit reference and an already-passing automated test (quick task 260827-isc, 2026-08-27). **WR-02 remains open** — the Postgres-catalog-level fix is real and applied live, but a new wire-level test found the managed Realtime service still delivers the withheld column, an infrastructure-level finding this session could not resolve. Per this report's own precondition, both warnings must resolve before status can flip to `passed`; only one has, so **status stays `human_needed`**. WR-03 (Hebrew pluralization), IN-01 (locale cookie `Secure` attribute), and IN-02 (doctor change-password translation timing) remain exactly as originally documented — untouched, non-blocking, carried forward.

---

_Verified: 2026-08-13T19:30:00Z_
_Verifier: Claude (gsd-verifier)_
_Re-verified: 2026-08-27T00:00:00Z (quick task 260827-isc) — WR-01 resolved, WR-02 partially addressed and still open, status unchanged at `human_needed`_
